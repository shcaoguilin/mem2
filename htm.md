...
- 对 该 激活 列(们) 的 每一个，我们 调整  　全部 该 潜在 synapse(们)的   　 紧结 值。跟 激活 输入 bit(们) 对齐的　 synapse(们)的 紧结 值　被 增加了。 跟 非激活 输入 bit(们) 对齐的　 synapse(们)的 紧结 值　被 降低了。 对 紧结 值(们) 的 修改 可能 改变 一些 synapse(们),从 合法 到 不合法、或 相反。
