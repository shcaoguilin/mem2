...
#### - synapse(译者注:权重)(们) 
- 分层时刻记忆 synapse(们) 有 二值 权重。生物学 synapse 有 可变 权重,单 他们 也是 部分 随机的，意思是说 一个 生物 神经元 不 依赖 精确的 synaptic 权重(们)。 在 分层时刻记忆(们) 的 分布 表示(们) 的 使用,加上,我们 的 dendrite 操作 模型,允许 我们 赋予 二值 权重(们)  给 分层时刻记忆 synapse(们),不 带有 病态 作用。为了 建模,synapse(们) 的 形成、没形成,我们 用 两个 额外的 概念,从 神经科学,你 可能 不熟悉的。一个 是 潜在 synapse(们) 的 概念。 **这 表示  所有 axon(们),靠的 足够近 到 一个 dendrite 段,他们(axon和dendrite段) 能 潜在地 形成 一个 synapse。** 第二个 被 叫做 "紧结"。这 是 一个 标量 值,赋给 每一个 潜在 synapse。一个 synapse的 紧结 表示 一个axon 和 一个 dendrite 之间的 连接程度 的 范围。生物学地，这个范围 会 从 完全地 不连接的 synapse,到 开始 形成 一个 synapse 但 还 没 连接，到 一个 最小地 连接的 synapse，到 一个 极大地 完全的 连接的 synapse。一个 synapse 的 紧结 是 一个 标量值,变化 从 0.0 到 1.0。学习 包含 增加、减小 一个 synapse的 紧结。当 一个 synpase的 紧结 是 在 一个 阈值 之上，它 被 连接 以 权重 "1" 。 当 它 是 在 该 阈值 之下，它 断开 连接 以 权重 "0"。
