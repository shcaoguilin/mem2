...
#### - 预测 是 上下文 敏感的。
- 预测 是 基于 过去 什么 已经 出现了，和 现在 什么 正 出现。(译者注:预测是基于过去的输入和现在的输入)。因此 一个 输入 会 产生 不同的 预测,基于 先前的 上下文。一个 分层临时记忆 区 学着 去 用,像 需要的 一样的 许多 前置 上下文，并且 能 保持 上下文,在 短、长 时间 展开 (上)。这个 能力 叫 可变 顺序 记忆。例如，考虑 一个 记忆的 演讲,例如 Gettysburg Address。要 预测 下一个 词，只 知道 当前 词 是 很难 够的
